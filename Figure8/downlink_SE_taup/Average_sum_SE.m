...
%%
coherent_taup=zeros(length(FDT),K*nbrOfSetups);
noncoherent_taup=zeros(length(FDT),K*nbrOfSetups);

for i=1:length(FDT)
coherent_taup(i,:)=sort(reshape(SE_MR_analytical_tot_coherent(i,:,:),[K*nbrOfSetups 1]));
noncoherent_taup(i,:)=sort(reshape(SE_MR_analytical_tot_noncoherent(i,:,:),[K*nbrOfSetups 1]));
...
plot(FDT,coherent_taup(:,0.05*K*nbrOfSetups),'r-<','LineWidth',2);
plot(FDT,noncoherent_taup(:,0.05*K*nbrOfSetups),'b-<','LineWidth',2);

xlabel('Normalized Doppler Shift ($f_DT_s$)','Interpreter','latex');
ylabel('95%-likely Per-User Downlink SE (bit/s/Hz)','Interpreter','latex');
legend('coherent_taup','noncoherent_taup','Interpreter','latex');
