function [SE] = functionComputeSE_AP_uplink_simulation_SC(hhat,R,gamma_kl,K,L,N,tau_c,tau_d,Pmax,eta,rho,nbrOfRealizations)
...
SINR=zeros(nbrOfRealizations,K,L,tau_d);
     ...
     for s=1:nbrOfRealizations
         for k=1:K
             for l=1:L
                 test1(s,k,l)=eta(k)*abs(hhat(:,s,k,l)'*hhat(:,s,k,l))^2;
                 term5(s,k,l)=sigma2*hhat(:,s,k,l)'*eye(N)*hhat(:,s,k,l);
                 for i=1:K
                     test3(s,k,i,l)=eta(i)*hhat(:,s,k,l)'*R(:,:,i,l)*hhat(:,s,k,l);
                     test4(s,k,i,l)=eta(i)*hhat(:,s,k,l)'*gamma_kl(:,:,i,l)*hhat(:,s,k,l);
                     if i==k
                        test2(s,k,i,l)=0;
                     else
                        test2(s,k,i,l)=eta(i)*abs(hhat(:,s,k,l)'*hhat(:,s,i,l))^2;
                     end
                 end
